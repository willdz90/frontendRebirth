...
async function fetchingData({ page=0, name, type, location, gender , age, size}) {
    ...
    const dataTest = await axios.get(`${SERVER}/?page=${page}&name=${nameFilter}&type=${typeFilter}&age=${ageFilter}&gender=${genderFilter}&size=${sizeFilter}&location=${locationFilter}`);
...
async function fetchingFavorites({mail}){
    ...
    const  dataFavorites  = await axios.get(`${SERVER}/user/Favs/${mail}`)
    return dataFavorites
}

async function favoritesPetsDetails({id}){
    console.log('mail :>> ', id);
    const dataFavorites = await axios.get(`${SERVER}/pets/${id}`)
